    ...
    **Critical principle:** This process must produce identical results when run multiple times with the same input.
    ...
    1. Create a plan to evaluate each input column against reference columns and match the columns according to the rules below
    2. Follow the decision framework step-by-step, applying hard constraints first to filter candidates, then using semantic judgment and tiebreakers to finalize matches
    3. Document reasoning and evidence for each match, including confidence scores based on defined criteria
    4. Use tools if necessary to ensure accuracy and consistency
    ---
    ...
    | identifier | identifier |Identifier patterns ONLY match identifier patterns but check cardinality and  uniqueness constraints|
    ...
    **Application:** Reject matches that violate cardinality rules, regardless of semantic similarity.
    ...
    If in doubt as to measning use tools to check, don't guess
